...
upload_content() {
    ...
    # Prompt user for input
    read -p "Enter the folder share to upload content from (e.g., nextcloud): " folder_share
...
clear_variables() {
    # Clear stored variables and delete share link files
    unset folder_share share_link share_id download_url
    rm -f */share_link_download.txt */share_link_upload.txt
    echo "All stored variables have been cleared and share link files have been deleted."
    read -p "Press enter to get back to the main menu: " enter
    main_menu
}
